...
class Customer {
    ...
    mob_number;
    acc_number;
    constructor(fName, lName, age, gender, mobile, accNum) {
...
async function bankService(bank) {
    do {
        ...
        if (service.select == "View Balance") {
            ...
            if (account) {
                let name = myBank.customer.find((item) => item.acc_number == account?.accNumber);
                console.log(`\nDear ${chalk.italic.green(name?.firstName)} ${chalk.italic.green(name?.lastName)} \nYour Account Balance is ${chalk.bold.blueBright(`$${account.balance}\n`)}`);
            }
        }
        //Cash Withdrawal
